...
# Pooling_datastore = SQLAlchemyUserDatastore(db, User, Role)
...
class UserView(MyModelView):
    column_editable_list = ['email','password','first_name', 'last_name']
    column_searchable_list = column_editable_list
    column_exclude_list = ['password']
    # form_excluded_columns = column_exclude_list
    column_details_exclude_list = column_exclude_list
    column_filters = column_editable_list
    # column_formatters = dict(email = lambda v,c,m,p: m.email[200:210])


class PoolingView(MyModelView):
...
class PostAdmin(ModelView):
    ...
    create_template = 'modify.html'  # 指定创建记录的模板
    edit_template = 'modify.html'  # 指定编辑记录的模板
    column_editable_list = ['id','title','unit','publish_time', 'provinces','cities','url','score','resume','keywords','content','is_checked']
    column_searchable_list = column_editable_list
    column_exclude_list = ['hash_code','resume','url','provinces','cities','url','keywords','score']
    # 'content',
...
admin.add_view(PoolingView(Pooling, db.session, menu_icon_type='fa', menu_icon_value='fa-wrench',name = "修稿文稿"))
admin.add_view(KGraphView(menu_icon_type='fa', menu_icon_value='fa-wrench',name = "知识图谱"))
admin.add_view(CrawlerView(name="数据采集", menu_icon_type='fa', menu_icon_value='fa-houzz'))
admin.add_view(PostAdmin(Post, db.session,menu_icon_type='fa', menu_icon_value='fa-wrench',name = "编辑发布"))
admin.add_view(tryView(menu_icon_type='fa', menu_icon_value='fa-wrench',name = "尝试"))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(database_path):
        build_sample_db()
    # Start app
    # app.run(host = "10.10.110.65", port =5000,debug=True)
